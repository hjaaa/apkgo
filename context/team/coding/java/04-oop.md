...
本项目统一使用 Lombok：`@Data` / `@Getter` / `@Setter` / `@ToString` 等注解生成的方法视为满足本章对 POJO 的 getter / setter / toString 相关要求，禁止手写样板方法；"getter / setter 中不要增加业务逻辑"等约束同样适用（即不要为加逻辑而放弃 Lombok 改为手写）。
