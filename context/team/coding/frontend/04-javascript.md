...
JavaScript 编码规约主要包含编码风格、语言特性、注释、命名等几个部分。本规约面向的 ECMAScript 语言版本是 ES6+。
...
- 1.2.1 【强制】语句结尾不写分号，由 Prettier 统一处理（`semi: false`）。

  当语句以 `(`、`[`、反引号等字符开头可能触发自动分号插入（ASI）歧义时，Prettier 会自动在行首补必要的分号，无需手动处理。本文其余示例代码中如出现行尾分号，一律以 Prettier 格式化结果为准。
